...
def fingerprint_model(input_shape):
    ...
    # ResNet
    x_shortcut = Conv2D(72, kernel_size=(1, 1), strides=(1, 1), padding='same')(x)
    x = inception_block_1(x)
    x = keras.layers.add([x, x_shortcut])
    x = MaxPooling2D(pool_size=(2, 2), strides=(2, 2), padding='valid')(x)

    # ResNet
    x_shortcut = Conv2D(144, kernel_size=(1, 1), strides=(1, 1), padding='same')(x)
    ...
    # ResNet
    x_shortcut = Conv2D(288, kernel_size=(1, 1), strides=(1, 1), padding='same')(x)
...
history = myModel.fit(x=X_train_orig, y=Y_train_orig, validation_split=0.09090909, batch_size=64, epochs=30,
