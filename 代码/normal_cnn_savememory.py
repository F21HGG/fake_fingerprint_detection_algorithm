...
def HappyModel(input_shape):
    ...
    X = BatchNormalization(axis=3)(X_input)         # BN

    X = ZeroPadding2D(padding=(1, 1))(X)  #padding
...
happyModel.fit(x=X_train_orig, y=Y_train_orig, validation_split=0.09090909, batch_size=64, epochs=20, callbacks=callbacks_list)
happyModel = load_model(filepath)
preds = happyModel.evaluate(x=X_test_orig, y=Y_test_orig)
